...
/**
 * Directory leaf for one managed worktree: `<repo basename>-<branch>`.
 *
 * The parent group is an opaque repository hash, so the project name is what
 * makes a leaf identifiable on its own. Both halves are budgeted to keep a
 * leaf ≤65 characters, and the repo half keeps its full text up to 24: a
 * truncated tail is exactly the part that distinguishes two repositories
 * (`dsh-plugin-pyrun` vs `dsh-persist-reasoning` share their first ten
 * characters). Uniqueness is not needed — every repository owns its own hash
 * group — so `uniquePath` stays the only collision authority.
 */
export function deriveWorktreeLeaf(mainRoot, branch) {
  const cut = (value, max) => {
    const trimmed = String(value).slice(0, max).replace(/[-.]+$/, '');
    return trimmed || 'wt';
  };
  return `${cut(slugify(basename(mainRoot)), 24)}-${cut(slugify(branch), 40)}`;
}
...
export async function createWorktree(opts) {
  ...
  if (intent === 'branch-off') {
    // `branchName` is the form's required name; `slug` remains the API
    // fallback (older clients send only a slug) and a slugless caller gets a
    // server-side mnemonic. Whatever the source, the name is final — this
    // plugin never renames a branch after creation (ADR 0014).
  ...
  const finalPath = await uniquePath(join(root, deriveWorktreeLeaf(mainRoot, finalBranch)));

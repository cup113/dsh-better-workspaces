...
/* ---- hero control ---- */
.dsh-bw-hero { display: inline-flex; align-items: center; gap: 2px; row-gap: 4px; min-width: 0; max-width: 100%; flex-wrap: wrap; position: relative; }
.dsh-bw-hero-btn { max-width: 240px; min-width: 0; min-height: 28px; color: var(--dsh-bw-primary-label); cursor: pointer; background: 0 0; border: none; border-radius: 16px; align-items: center; gap: 4px; padding: 0 8px; font-size: 13px; font-weight: 500; line-height: 20px; display: inline-flex; }
...
.dsh-bw-hero-error { color: var(--dsh-bw-red); font-size: 12px; line-height: 18px; max-width: 320px; min-width: 0; overflow-wrap: anywhere; padding: 0 8px; }
/* inside the wrapped hero row an error never shares a line with controls: the
 * long host messages (worktree/rollback receipts) read as one block, and the
 * row's flex items can shrink without squeezing the text to one char a line */
.dsh-bw-hero > .dsh-bw-hero-error { flex-basis: 100%; max-width: none; }
...
`;
		...
		const zh = {
			...
			"hero.name": "分支名",
			"hero.namePlaceholder": "分支名（点骰子随机）",
			"hero.diceName": "随机命名",
			"hero.nameInvalid": "分支名不合法",
			"hero.prNameLocked": "PR 检出沿用该 PR 的分支名",
			...
			"forge.issuesFailed": "Issue 列表不可用:{message}",
		...
		const en = {
			...
			"hero.name": "Branch name",
			"hero.namePlaceholder": "Branch name (or roll the dice)",
			"hero.diceName": "Roll a random name",
			"hero.nameInvalid": "That branch name is not valid",
			"hero.prNameLocked": "A PR checkout uses the pull request's branch name",
			...
			"forge.issuesFailed": "Issue list unavailable: {message}",
		...
		/**
		 * The exact request one staged form submits (ADR 0014): the name is
		 * required for a branch-off and the picked base travels WITH it — the
		 * pre-form shape dropped the base whenever a name was present, so a
		 * named staging always cut from the default branch. `slug` is gone: the
		 * directory leaf now derives from `<repo>-<branch>` on the host, so a
		 * client-minted mnemonic would only name a directory nothing reads.
		 * The `selection` half is the idempotency key; `body` is what is retried
		 * verbatim after a lost response.
		 */
		function stagedCreateRequest({ cwd, sourceId, sourceTitle, pull, name, baseRef }) {
			const selection = {
				cwd,
				intent: pull ? "pr-checkout" : "branch-off",
				branchName: pull ? null : String(name == null ? "" : name).trim(),
				base: pull ? null : baseRef || null,
				pull: pull ? {
					host: pull.host,
					owner: pull.owner,
					repo: pull.repo,
			...
			if (selection.pull) body.pull = selection.pull;
				...
				body.branchName = selection.branchName;
				if (selection.base) body.base = selection.base;
			}
		...
		/**
		 * Why the staged branch name cannot be used, or null when it can:
		 * mirrors the host's `isValidBranchName` (git check-ref-format) so the
		 * Create button refuses before a round trip; the host stays the
		 * authority (a 422 still lands in the hero error).
		 */
		function branchNameProblem(name) {
			const value = String(name == null ? "" : name).trim();
			if (value === "") return "empty";
			if (value === "@" || value.length > 255) return "invalid";
			if (/^[-/.]/.test(value) || /[/.]$/.test(value)) return "invalid";
			if (value.includes("..") || value.includes("//") || value.includes("@{")) return "invalid";
			if (value.split("/").some((part) => part === "" || part.startsWith(".") || part.endsWith(".lock"))) return "invalid";
		...
		function HeroControlResource(props) {
			...
			const [pullPick, setPullPick] = useState(null); // staging: PR-checkout row
			const [branchName, setBranchName] = useState(""); // form name (required)
						...
						apiGet("/pulls" + qs({ cwd }), token.controller.signal)
							.then((result) => {
								...
								if (result.ok) {
									...
									if (result.authState === "partial") {
									const message = result.error || t("forge.loadFailed");
									setPullsError(t(result.partialSide === "issues" ? "forge.issuesFailed" : "forge.pullsFailed", { message }));
								}
								} else {
									setPulls({ cwd, value: [] });
			...
			async function prepareStagedWorktree(baseBranch, explicit, pull) {
				...
				const named = String(explicit || branchName || "").trim();
				// defensive: the form's Create button is disabled in this state
				if (!pull && branchNameProblem(named) !== null) return;
				...
				const { selection, body } = stagedCreateRequest({
					cwd, sourceId, sourceTitle, pull, name: named,
					baseRef: baseBranch || branchPick || defaultBaseRef() || null,
				});
				const pending = heroCreationRequest(sourceId, selection, () => body);
				try {
			...
			/* staging form actions: the name is required, and the explicit Create
			 * click is the only creation trigger — picking a base branch or a PR
			 * row stays side-effect free (ADR 0014) */
			function createStaged() {
				if (busy) return;
				if (pullPick) { prepareStagedWorktree(null, null, pullPick); return; }
				prepareStagedWorktree(branchPick || null, branchName);
			}

			function createDisabled() {
				if (busy) return true;
				if (pullPick) return false;
				return branchNameProblem(branchName) !== null;
			}
			...
			return h(react.Fragment, null,
					...
					trigger: ({ open, toggle }) =>
						h("button", { type: "button", className: "dsh-bw-hero-btn", onClick: toggle, disabled: Boolean(busy), "aria-expanded": open },
							...
							h("span", { className: "dsh-bw-hero-label" },
								pullPick
									? "#" + pullPick.number + " " + (pullPick.title || "")
						...
						liveBranches === null
							...
							: h(MenuList, {
								...
								onSelect: (id) => {
									...
									close();
									// selection is side-effect free: only the Create
									// button creates (ADR 0014)
									const pr = prItems.find((entry) => entry.id === id);
									if (pr) { setPullPick(pr.item); setBranchPick(null); return; }
									setPullPick(null);
									setBranchPick(id);
								},
							}),
				...
				staging ? h("input", {
					className: "dsh-bw-hero-input", value: branchName, "aria-label": t("hero.name"),
					placeholder: t("hero.namePlaceholder"),
					disabled: Boolean(busy) || Boolean(pullPick),
					title: pullPick ? t("hero.prNameLocked") : t("hero.name"),
					onChange: (e) => setBranchName(e.target.value),
					onKeyDown: (e) => {
						if (e.key === "Enter") { e.preventDefault(); createStaged(); }
						else if (e.key === "Escape") { e.preventDefault(); resetStaging(); }
					},
				}) : null,
				staging ? h("button", {
					type: "button", className: "dsh-bw-hero-btn", "aria-label": t("hero.diceName"), title: t("hero.diceName"),
					disabled: Boolean(busy) || Boolean(pullPick),
					onClick: () => setBranchName(mnemonicSlug()),
				}, "\u2684") : null,
				staging ? h("button", {
					type: "button", className: "dsh-bw-btn dsh-bw-btn-primary", disabled: createDisabled(),
					onClick: () => createStaged(),
				}, busy || (pullPick ? t("hero.checkoutPr") : t("hero.create"))) : null,
				staging && !pullPick && branchNameProblem(branchName) === "invalid"
					? h("span", { className: "dsh-bw-hero-error", role: "alert" }, t("hero.nameInvalid")) : null,
